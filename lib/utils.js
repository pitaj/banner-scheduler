...
function parse(html){
	...
	for(x = 0; x < sections.length; x++){
		...
		thisData = {
			...
			"links": {
				class: config.urls.base + section.find("b > font > a").attr("href") || "",
				title: section.find("a").has("font").attr("href") || ""
			}
		};
...
function getHTML(ops, callback){
	...
	request.post({
		url: config.urls.base + config.urls.classList,
...
Module.getSelectOptions = function(callback){
	request({
		url: config.urls.base + config.urls.selectOptions
...
Module.getInstructors = function(options, callback){
	request({
		url: config.urls.base + config.urls.instructorList,
		qs: {
